...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SMUSH }
    ...
    address public sMUSH;
    uint public sMUSHQueue; // Delays change to sMUSH address
    ...
    /**
        @notice allow approved address to deposit an asset for MUSH
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint MUSH needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( MUSH ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn MUSH for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IMushERC20( MUSH ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sMUSH ).balanceOf( msg.sender ); // Can only borrow against sMUSH held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with MUSH
        @param _amount uint
     */
    function repayDebtWithMUSH( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IMushERC20( MUSH ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, MUSH, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( MUSH ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( MUSH ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns MUSH valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match MUSH decimals
            value_ = _amount.mul( 10 ** IERC20( MUSH ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SMUSH ) { // 9
            sMUSHQueue = block.number.add( blocksNeededForQueue );
        } else return false;
